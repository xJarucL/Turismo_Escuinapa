...
<head>
  ...
  <title>Detalles del Hotel | Turismo Escuinapa</title>
...
<body class="flex flex-col min-h-screen bg-gray-100 text-gray-800 font-serif">
  ...
  <main class="flex-grow px-6 py-10 max-w-7xl mx-auto">
    ...
    <a href="{{ route('lista_hoteles') }}" onclick="" class="inline-flex items-center justify-center w-10 h-10 bg-red-600 hover:bg-red-500 text-white rounded-full shadow mb-4" title="Volver">
    ...
    <div class="grid md:grid-cols-3 gap-6 text-sm mb-10">
      <p class="font-serif md:col-span-3">{{ $hotel->descripcion }}</p>
      <p><strong>Dirección:</strong> {{ $hotel->direccion }} </p>
      <p><strong>Contacto:</strong> {{ $hotel->contacto }} </p>
      @if (!empty($hotel->link_hotel))
      <p><strong>Enlace extra o reseñas:</strong>
        <a href="{{ $hotel->link_hotel }}" class="text-blue-600 hover:underline" target="_blank">
          {{ parse_url($hotel->link_hotel, PHP_URL_HOST) }}
        </a>
      </p>
      @endif
    </div>
    ...
    <form action="{{ route('hotel.seleccionar') }}" method="POST" onsubmit="return seleccionarHotelYRedirigir(this);">
        @csrf
        <input type="hidden" name="pk_hotel" value="{{ $hotel->pk_hotel }}">
        <button type="submit"
        class="flex items-center gap-2 px-3 py-2 rounded hover:bg-gray-200 transition text-gray-700 group text-sm"
        title="Editar imágenes del hotel" aria-label="Editar imágenes del hotel">
    ...
    <div
      ...
      @forelse ($hotel->imagenes as $imagen)
      <div
        ...
        <img src="{{ asset('storage/' . $imagen->ruta) }}" alt="Imagen del hotel" class="object-cover w-full h-full">
      </div>
      @empty
...
<script>
  // Redirigir al hotel seleccionado
        ...
        fetch("{{ route('hotel.seleccionar') }}", {
          ...
          .then(response => {
            if (response.ok) {
              ...
              alert("Error al seleccionar hotel.");
            }
          })
  ...
  function seleccionarHotelYRedirigir(form) {
    ...
    fetch(form.action, {
      ...
      if (response.ok) {
        window.location.href = "{{ route('hoteles.imagenes.cargar') }}";
      } else {
