...
<head>
    ...
    <title>Editar imágenes del hotel | Turismo Escuinapa</title>
...
<body class="bg-gray-100 text-gray-800 min-h-screen flex flex-col font-serif">
    ...
    <main class="flex-grow">
        <div class="max-w-xl mx-auto mt-10 mb-10 bg-white p-8 rounded-lg shadow-md border border-gray-200">
            <a href="{{ route('informacion_hotel') }}"
            ...
            <label class="block font-medium mb-2">Imágenes del hotel</label>

            @if ($hotel->imagenes->count())
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
                    @foreach ($hotel->imagenes as $imagen)
                        <div class="relative group border rounded overflow-hidden shadow bg-gray-100">
                            <img src="{{ asset('storage/' . $imagen->ruta) }}" alt="Imagen del hotel"
                                class="w-full h-32 object-cover transition-transform duration-300 group-hover:scale-105">

                            <form action="{{ route('hoteles.imagenes.eliminar', ['imagenId' => $imagen->pk_img_hotel]) }}"
                                method="POST"
            ...
            @else
                <p class="font-serif text-sm text-gray-500 italic mb-6">No hay imágenes cargadas para este hotel.</p>
            @endif

            <!-- Dropzone -->
            <div class="mb-6">
                <label class="block font-medium mb-2">Agregar imágenes al hotel</label>
                <form action="{{ route('hoteles.imagenes.subir', ['hotelId' => $hotel->pk_hotel]) }}"
                    method="POST"
                ...
                <p class="font-serif text-xs text-gray-500 mt-1">Puedes subir varias imágenes del hotel.</p>
            </div>

            <input type="hidden" id="input-hotel-id" value="{{ $hotel->pk_hotel }}">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            const hotelId = document.getElementById('input-hotel-id').value;
            ...
            const myDropzone = new Dropzone("#dropzoneImagenes", {
                ...
                removedfile: function (file) {
                    ...
                    fetch(`/hoteles/imagenes/${imagenId}/eliminar`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': '{{ csrf_token() }}',
