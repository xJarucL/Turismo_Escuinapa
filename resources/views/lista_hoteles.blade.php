...
<head>
    ...
    <title>Hoteles | Turismo Escuinapa</title>
...
<body class="flex flex-col min-h-screen bg-white text-gray-900 font-serif">
    ...
    <main class="flex-grow px-4 py-10 w-full">
        ...
        <div class="flex justify-center items-center mb-6">
            <h1 class="text-3xl font-serif">Hoteles</h1>
        </div>

        <div class="flex justify-center mb-10">
            <div class="relative w-full max-w-md">
                <input id="buscador-hoteles" type="text" placeholder="Buscar evento..." class="w-full pl-4 pr-10 py-2 border border-red-400 rounded-full focus:outline-none focus:ring-2 focus:ring-red-400">
        ...
        <div id="contenedor-hoteles" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-w-7xl mx-auto px-4">
            @foreach ($hoteles as $hotel)
            <div class="hotel-card border rounded-lg p-3 text-center shadow-xs transition-all duration-300 hover:shadow-md hover:-translate-y-1 mx-auto w-full max-w-[280px]">
                <div class="bg-gray-100 rounded-md aspect-square mb-2 overflow-hidden">
                    <a href="{{route('informacion_hotel')}}" class="enlace-hotel block w-full h-full" data-id="{{ $hotel->pk_hotel }}">
                        <img src="{{ asset('storage/' . $hotel->img_hotel) }}"
                            alt="Hotel"
                            class="w-full h-full object-cover object-center transition-transform duration-500 hover:scale-105" />
                    </a>
                </div>
                <a href="{{route('informacion_hotel')}}" class="enlace-hotel block" data-id="{{ $hotel->pk_hotel }}">
                    <h3 class="font-medium text-lg nombre-hotel hover:text-red-600 transition-colors duration-300 mb-0.5 line-clamp-1">
                        {{ $hotel->nom_hotel }}
                    </h3>
                </a>
                <p class="font-serif text-xs text-gray-500 mb-1.5 line-clamp-2">
                    {{ \Illuminate\Support\Str::limit($hotel->descripcion, 50, '...') }}
                </p>
            ...
            <main class="flex-grow px-6 py-10 w-full max-w-7xl mx-auto">
                ...
                <div class="flex justify-center mb-10">
                    <div class="relative w-full max-w-md">
                        <input id="buscador-hoteles-publico" type="text" placeholder="Buscar hotel..."
                ...
                <div id="contenedor-hoteles" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-w-7xl mx-auto px-6">
                    @foreach ($hoteles_public as $hotel)
                    <div class="hotel-card border rounded-lg p-3 text-center shadow-xs transition-all duration-300 hover:shadow-md hover:-translate-y-1 mx-auto w-full max-w-[280px]">
                        <div class="bg-gray-100 rounded-md aspect-square mb-2 overflow-hidden">
                            <a href="{{route('informacion_hotel')}}" class="enlace-hotel block w-full h-full" data-id="{{ $hotel->pk_hotel }}">
                                <img src="{{ asset('storage/' . $hotel->img_hotel) }}"
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('.enlace-hotel').forEach(function(enlace) {
            enlace.addEventListener('click', function(e) {
                e.preventDefault();
                const hotelId = this.dataset.id;

                fetch("{{ route('hotel.seleccionar') }}", {
                        method: 'POST',
                        headers: {
                    ...
                    .then(response => {
                        if (response.ok) {
                            window.location.href = "{{ route('informacion_hotel') }}";
                        } else {
                            console.error('Error al seleccionar el hotel');
                        }
                    })
                    .catch(error => {
    ...
    // Cambiar estatus, mostrar hoteles activos e inactivos y buscar hoteles
    document.addEventListener("DOMContentLoaded", function() {
        const csrfToken = '{{ csrf_token() }}';
        const switchEstatus = document.getElementById('switch-estatus');
        const mensajeSinHoteles = document.getElementById('mensaje-sin-hoteles');
        const buscador = document.getElementById("buscador-hoteles");
        ...
        function filtrarHoteles() {
            ...
            if (hotelesVisibles === 0) {
                if (!mensajeExistente) {
                    const mensaje = document.createElement("p");
                    mensaje.id = "sin-resultados";
                    mensaje.className = "text-center col-span-full text-gray-500";
                    mensaje.textContent = "No se encontraron hoteles.";
                    contenedorHoteles.appendChild(mensaje);
                }
            } else {
                if (mensajeExistente) mensajeExistente.remove();
            }

            if (mensajeSinHoteles) {
                if (hotelesVisibles === 0) {
                    mensajeSinHoteles.classList.remove('hidden');
                } else {
                    mensajeSinHoteles.classList.add('hidden');
                }
            }
        }

        switchEstatus.addEventListener('change', filtrarHoteles);
        buscador.addEventListener('input', filtrarHoteles);
        ...
        document.querySelectorAll('.btn-cambiar-estatus').forEach(button => {
            button.addEventListener('click', function() {
                ...
                fetch(`/hotel/cambiar-estatus/${id}`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
    ...
    document.addEventListener("DOMContentLoaded", function() {
        const buscadorPublico = document.getElementById("buscador-hoteles-publico");
        const contenedorHotelesPublico = document.getElementById("contenedor-hoteles");
        ...
        if (buscadorPublico) {
            buscadorPublico.addEventListener("input", function() {
                ...
                contenedorHotelesPublico.querySelectorAll('.hotel-card').forEach(card => {
                    const nombre = card.querySelector('.nombre-hotel').textContent.toLowerCase();
                ...
                if (visibles === 0) {
                    if (!mensaje) {
                        ...
                        mensaje.textContent = "No se encontraron hoteles.";
                        contenedorHotelesPublico.appendChild(mensaje);
                    }
                } else {
