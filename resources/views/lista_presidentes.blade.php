...
<head>
    ...
    <title>Presidentes | Turismo Escuinapa</title>
...
<body class="flex flex-col min-h-screen bg-white text-gray-900 font-serif">
    ...
    <main class="flex-grow px-4 py-10 w-full">
        <div class="max-w-7xl mx-auto px-4 flex justify-between">
            ...
            @endif
            <a href="{{ route('registro_presidente') }}"
        ...
        <div class="flex justify-center items-center mb-6">
            <h1 class="text-3xl font-serif">Presidentes Municipales</h1>
        </div>

        <div class="flex justify-center mb-10">
            <div class="relative w-full max-w-md">
                <input id="buscador-presidente" type="text" placeholder="Buscar presidente municipal..." class="w-full pl-4 pr-10 py-2 border border-red-400 rounded-full focus:outline-none focus:ring-2 focus:ring-red-400">
        ...
        <div id="contenedor-presidentes" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-w-7xl mx-auto px-4"> @foreach ($presidentes as $presidente)
            <!-- Contenedor de imagen - versión compacta -->
            <div class="presidente-card border rounded-lg p-3 text-center shadow-xs transition-all duration-300 hover:shadow-md hover:-translate-y-1 mx-auto w-full max-w-[280px]">
                <div class="bg-gray-100 rounded-md aspect-square mb-2 overflow-hidden">
                    <a href="{{ route('informacion_presidente') }}" class="enlace-presidente block w-full h-full" data-id="{{ $presidente->pk_presidente }}">
                        <img src="{{ asset('storage/' . $presidente->img_presidente) }}"
                            alt="{{ $presidente->nombre }}"
                            class="w-full h-full object-cover object-center transition-transform duration-500 hover:scale-105" />
                    </a>
                </div>
                <a href="{{ route('informacion_presidente') }}" class="enlace-presidente block" data-id="{{ $presidente->pk_presidente }}">
                    <h3 class="font-medium text-lg nombre-presidente hover:text-red-600 transition-colors duration-300 mb-0.5 line-clamp-1">
                        {{ $presidente->nombre }}
                    </h3>
                </a>
                <p class="font-serif text-xs text-gray-500 mb-1.5 line-clamp-2">
                    {{ \Carbon\Carbon::parse($presidente->fec_inicio)->format('d/m/Y') }} -
                    {{ \Carbon\Carbon::parse($presidente->fec_fin)->format('d/m/Y') }}
                </p>

                @if ($presidente->estatus == 1)
                <button
                    ...
                    data-id="{{ $presidente->pk_presidente }}"
                    data-estatus="0">
                    Desactivar
                </button>
                @else
        ...
        <main class="flex-grow px-6 py-10 w-full max-w-7xl mx-auto">
            ...
            <div class="flex justify-center mb-10">
                <div class="relative w-full max-w-md">
                    <input id="buscador-presidente-publico" type="text" placeholder="Buscar presidente municipal..."
            ...
            <div id="contenedor-presidentes" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-w-7xl mx-auto px-6">
                @foreach ($presidentes_public as $presidente)
                <div class="presidente-card border rounded-lg p-3 text-center shadow-xs transition-all duration-300 hover:shadow-md hover:-translate-y-1 mx-auto w-full max-w-[280px]">
                    <div class="bg-gray-100 rounded-md aspect-square mb-2 overflow-hidden">
                        <a href="{{ route('informacion_presidente') }}" class="enlace-presidente block w-full h-full" data-id="{{ $presidente->pk_presidente }}">
                            <img src="{{ asset('storage/' . $presidente->img_presidente) }}"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('.enlace-presidente').forEach(function(enlace) {
            enlace.addEventListener('click', function(e) {
                e.preventDefault();

                const presidenteId = this.dataset.id;

                fetch("{{ route('presidente.seleccionar') }}", {
                        method: 'POST',
                        headers: {
                        ...
                        body: JSON.stringify({
                            pk_presidente: presidenteId
                        })
                    })
                    .then(response => {
                        if (response.ok) {
                            window.location.href = "{{ route('informacion_presidente') }}";
                        } else {
                            alert("Error al seleccionar presidente.");
                        }
                    })
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const mensajeSinPresidentes = document.getElementById('mensaje-sin-presidentes');
        const buscador = document.getElementById("buscador-presidente");
        const contenedorPresidentes = document.getElementById("contenedor-presidentes");

        function filtrarPresidentes() {
            const mostrarInactivos = switchEstatus.checked;
            const query = buscador.value.toLowerCase().trim();
            let presidentesVisibles = 0;

            document.querySelectorAll('.presidente-card').forEach(card => {
                const boton = card.querySelector('.btn-cambiar-estatus');
                const estatus = boton.dataset.estatus === "0" ? "1" : "0";
                const nombre = card.querySelector(".nombre-presidente").textContent.toLowerCase();
                ...
                if (visible) presidentesVisibles++;
            ...
            if (presidentesVisibles === 0) {
                if (!mensajeExistente) {
                    const mensaje = document.createElement("p");
                    mensaje.id = "sin-resultados";
                    mensaje.className = "text-center col-span-full text-gray-500";
                    mensaje.textContent = "No se encontraron presidentes.";
                    contenedorPresidentes.appendChild(mensaje);
                }
            } else {
                if (mensajeExistente) mensajeExistente.remove();
            }

            if (mensajeSinPresidentes) {
                if (presidentesVisibles === 0) {
                    mensajeSinPresidentes.classList.remove('hidden');
                } else {
                    mensajeSinPresidentes.classList.add('hidden');
                }
            }
        }

        switchEstatus.addEventListener('change', filtrarPresidentes);
        buscador.addEventListener('input', filtrarPresidentes);
        ...
        document.querySelectorAll('.btn-cambiar-estatus').forEach(button => {
            button.addEventListener('click', function() {
                ...
                fetch(`/presidente/cambiar-estatus/${id}`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
    ...
    document.addEventListener("DOMContentLoaded", function() {
        const buscadorPublico = document.getElementById("buscador-presidente-publico");
        const contenedorPublico = document.getElementById("contenedor-presidentes");
        ...
        if (buscadorPublico && contenedorPublico) {
            buscadorPublico.addEventListener("input", function() {
                ...
                contenedorPublico.querySelectorAll('.presidente-card').forEach(card => {
                    const nombre = card.querySelector('.nombre-presidente').textContent.toLowerCase();
