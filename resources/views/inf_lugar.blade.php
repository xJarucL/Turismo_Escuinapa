...
<head>
  ...
  <title>Detalles del Lugar | Turismo Escuinapa</title>
...
<body class="flex flex-col min-h-screen bg-gray-100 text-gray-800 font-serif">
  ...
  <main class="flex-grow px-6 py-10 max-w-7xl mx-auto overflow-x-hidden">
    ...
    <div class="grid md:grid-cols-3 gap-6 text-sm mb-10">
      ...
      @if (!empty($lugar->url_google_resena))
      <p><strong>Enlace extra o reseñas:</strong>
        <a href="{{ $lugar->url_google_resena }}" class="text-blue-600 hover:underline" target="_blank">
          {{ $lugar->url_google_resena }}
        </a>
      </p>
      @endif
    ...
    <form action="{{ route('lugar.seleccionar') }}" method="POST" onsubmit="return seleccionarLugarYRedirigir(this);">
        @csrf
        <input type="hidden" name="pk_lugar_interes" value="{{ $lugar->pk_lugar_interes }}">
        <button
        type="submit"
    ...
    <div
      ...
      @forelse ($lugar->imagenes as $imagen)
      <div
        ...
        <img src="{{ asset('storage/' . $imagen->ruta) }}" alt="Imagen del lugar" class="object-cover w-full h-full">
      </div>
      @empty
      <p class="font-serif col-span-3 text-gray-500">No hay imágenes disponibles para este lugar.</p>
      @endforelse
...
<script>
  ...
  function seleccionarLugarYRedirigir(form) {
    ...
    fetch(form.action, {
      ...
      if (response.ok) {
        window.location.href = "{{ route('lugares.imagenes.cargar') }}";
      } else {
        alert("Error al seleccionar el lugar.");
      }
    });
