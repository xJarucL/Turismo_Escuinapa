...
<head>
  ...
  <title>Detalles del platillo de comida | Turismo Escuinapa</title>
...
<body class="flex flex-col min-h-screen bg-gray-100 text-gray-800 font-serif">
  ...
  <main class="flex-grow px-6 py-10 max-w-7xl mx-auto overflow-x-hidden">
    ...
    <div class="flex flex-row flex-nowrap justify-between items-center mb-8 gap-4 max-w-full overflow-hidden">
        ...
        @auth
        <a href="{{ route('editar_comida_tipica') }}"
    ...
    <!-- Descripción -->
    <div class="mb-10">
      <p class="font-serif text-base text-gray-700 leading-relaxed whitespace-pre-line break-words max-w-full">
        {{ $comida->descripcion }}
      </p>
    </div>
    ...
    <div class="grid md:grid-cols-3 gap-6 text-sm mb-10">
      <p><strong>Categoría:</strong> {{ $comida->cat_comida->nom_cat }}</p>
    </div>
    ...
    <form action="{{ route('comida.seleccionar') }}" method="POST" onsubmit="return seleccionarComidaYRedirigir(this);">
        @csrf
        <input type="hidden" name="pk_comida_tipica" value="{{ $comida->pk_comida_tipica }}">
        <button
        type="submit"
        class="flex items-center gap-2 px-3 py-2 rounded hover:bg-gray-200 transition text-gray-700 group"
        title="Editar imágenes del platillo de comida"
    ...
    <div
      ...
      @forelse ($comida->imagenes as $imagen)
      <div
        ...
        <img src="{{ asset('storage/' . $imagen->ruta) }}" alt="Imagen del platillo de comida" class="object-cover w-full h-full">
      </div>
      @empty
      <p class="font-serif col-span-3 text-gray-500">No hay imágenes disponibles para este platillo de comida.</p>
      @endforelse
...
<script>
    // Redirigir al comida seleccionada
        ...
        fetch("{{ route('comida.seleccionar') }}", {
          ...
          .then(response => {
            if (response.ok) {
              ...
              alert("Error al seleccionar la comida.");
            }
          })
  ...
  function seleccionarComidaYRedirigir(form) {
    ...
    fetch(form.action, {
      ...
      if (response.ok) {
        window.location.href = "{{ route('comidas.imagenes.cargar') }}";
      } else {
